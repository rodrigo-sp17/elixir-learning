defmodule MSupervisor do
  @moduledoc """
  Documentation for `MSupervisor`.
  """
  use GenServer
  ...
  def handle_call({:terminate_child, pid}, _from, state) do
    case terminate_child(pid) do
      :ok ->
        ...
        {:reply, "Terminated child #{inspect pid}", new_state}
      :error ->
        {:reply, {:error, "Error terminating child #{inspect pid}"}, state}
    end
  end
    ...
    case Map.fetch(state, old_pid) do
      {:ok, old_spec} ->
        case restart_child(old_pid, child_spec) do
          {:ok, {pid, child_spec}} ->
            new_state = state |> Map.delete(pid) |> Map.put(pid, child_spec)
            {:reply, {:ok, pid}, new_state}
          :error ->
            {:reply, {:error, "Error restarting child #{inspect old_pid}"}, state}
        end
      _ ->
        {:reply, :ok, state}
    end
  ...
  def handle_info({:EXIT, from, _reason}, state) do
    case Map.fetch(state, from) do
      {:ok, child_specs} ->
        case restart_child(from, child_specs) do
          {:ok, {new_pid, child_spec}} ->
            new_state = state |> Map.delete(from) |> Map.put(new_pid, child_spec)
            {:noreply, new_state}
          :error ->
            {:noreply, state}
        end
